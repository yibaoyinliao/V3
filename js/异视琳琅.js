var rule = {
    ...
    url: '/vodshow-{cateId}/page/{catePg}/',
...
"3":[
  ...
  {"n":"日本","v":"/area/日本"}]}]
    ...
    推荐:'',
    double:true,
   一级:'.vodlist&&.pack-ykpack;.vodname&&a&&Text;.eclazy&&data-original;.pack-prb&&Text;a&&href',
   //一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&.vodlist&&.pack-ykpack');tabs.forEach(function(it){var  pz=pdfh(it,'a&&title');var ps=pdfh(it,'.pack-prb.hidden&&Text');var img=pd(it,'div&&data-original');var timer=pdfh(it,'.pack-subtitle&&Text');var url=pd(it,'a&&href');items.push({desc:timer+'  '+ps,title:pz,pic_url:img,url:url})});setResult(items);",
   二级:{
          ...
          tabs:"li.this",
          lists:'.play_source_tab:eq(#id)&&a',   //显示直播信号数量。gt是指直播信号数量从-1开始第几个，li是指直播信号数量总共有几个。
          list_url:'a&&href'
         },
    搜索:'',
}